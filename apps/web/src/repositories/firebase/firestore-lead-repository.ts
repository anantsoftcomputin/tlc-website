...
import type {
  ...
  LeadRecord,
  LeadRepository,
} from "@/repositories/interfaces/lead-repository";
...
function canonicalStatus(value: unknown): Lead["status"] {
  if (leadStatuses.includes(value as Lead["status"]))
    return value as Lead["status"];
  const legacy: Record<string, Lead["status"]> = {
    new_lead: "new",
    requirement_received: "contacted",
    itinerary_preparation: "quoted",
    quote_sent: "quoted",
...
function mapLead(
  ...
  const priority = leadPriorities.includes(data.priority)
    ? data.priority
    : "normal";
  const destinations = Array.isArray(data.requirement?.destinations)
    ...
    : Array.isArray(data.destinationIds)
      ? data.destinationIds.map(String)
      : [];
  return {
    id: document.id,
    ...
    requirement: {
      destinations,
      ...(data.requirement?.startDate
        ? { startDate: String(data.requirement.startDate) }
        : {}),
      ...(data.requirement?.endDate
        ? { endDate: String(data.requirement.endDate) }
        : {}),
      ...
      ...(typeof data.requirement?.budgetMin === "number"
        ? { budgetMin: data.requirement.budgetMin }
        : {}),
      ...(typeof data.requirement?.budgetMax === "number"
        ? { budgetMax: data.requirement.budgetMax }
        : {}),
      preferences: Array.isArray(data.requirement?.preferences)
        ? data.requirement.preferences.map(String)
        : [],
      ...(data.requirement?.tripBrief
        ? { tripBrief: data.requirement.tripBrief }
        : {}),
      ...(Array.isArray(data.requirement?.travellers)
        ? { travellers: data.requirement.travellers }
        : {}),
    ...
    sla: {
      ...
      ...(data.sla?.firstResponseAt
        ? { firstResponseAt: toIso(data.sla.firstResponseAt) }
        : {}),
      ...(data.sla?.nextFollowUpAt || data.nextFollowUpAt
        ? {
            nextFollowUpAt: toIso(
              data.sla?.nextFollowUpAt || data.nextFollowUpAt,
            ),
          }
        : {}),
    },
    ageDays: Number.isInteger(data.ageDays)
      ? data.ageDays
      : Math.max(
          0,
          Math.floor(
            (Date.now() - new Date(toIso(data.createdAt)).getTime()) /
              86_400_000,
          ),
        ),
    flags: Array.isArray(data.flags) ? data.flags.map(String) : [],
    title: String(
      data.title || `${destinations[0] || "Custom holiday"} enquiry`,
    ),
...
function mapActivity(
  ...
  return {
    ...
    by: String(data.by || "system"),
    ts: toIso(data.ts),
    attachments: Array.isArray(data.attachments) ? data.attachments : [],
    createdAt: toIso(data.createdAt),
    updatedAt: toIso(data.updatedAt),
    createdBy: String(data.createdBy || data.by || "system"),
    updatedBy: String(data.updatedBy || data.by || "system"),
  } as LeadActivity;
}

function mapCustomer(
  snapshot: FirebaseFirestore.DocumentSnapshot<DocumentData>,
): LeadCustomerSummary | null {
...
export class FirestoreLeadRepository implements LeadRepository {
  ...
  constructor(
    private readonly orgId = "tlc-vacations",
  ...
  async listLeads(limit = 150): Promise<LeadListItem[]> {
    ...
    if (!this.viewer.canViewAll)
      query = query.where("assignedUid", "==", this.viewer.uid);
    ...
    const customerIds = [
      ...new Set(leads.map((lead) => lead.customerId).filter(Boolean)),
    ];
    ...
    const names = new Map(
      customers
        ...
        .map((item) => [
          item.id,
          String(item.data()?.name || "Unknown traveller"),
        ]),
    );
    return leads.map((lead) => ({
      ...lead,
      customerName: names.get(lead.customerId) || "Traveller not linked",
    }));
  }
  ...
  async getLead(leadId: string): Promise<LeadDetail | null> {
    ...
    if (!this.viewer.canViewAll && lead.assignedUid !== this.viewer.uid)
      return null;
    ...
    const customer =
      customerSnapshot.data()?.orgId === this.orgId
        ? mapCustomer(customerSnapshot)
        : null;
    const activities = activitySnapshot.docs
      .filter((item) => item.data().orgId === this.orgId)
      .map(mapActivity);
  ...
  async listAssignees(): Promise<LeadAssignee[]> {
    ...
    return snapshot.docs
      ...
      .map((item) => ({
        uid: item.id,
        displayName: String(
          item.data().displayName || item.data().email || "Team member",
        ),
        email: String(item.data().email || ""),
        role: item.data().role as UserRole,
      }))
