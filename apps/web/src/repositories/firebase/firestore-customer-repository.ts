  ...
  return {
    ...
    ...(data.profile
      ? {
          profile: {
            ...data.profile,
            computedAt: toIso(data.profile.computedAt),
          },
        }
      : {}),
    ...
    ...(data.lastActivityAt
      ...
      : {}),
    mergedFrom: Array.isArray(data.mergedFrom)
      ? data.mergedFrom.map(String)
      : [],
...
function mapNested<T>(document: QueryDocumentSnapshot<DocumentData>): T {
  ...
  return {
    ...
    createdAt: toIso(data.createdAt),
    updatedAt: toIso(data.updatedAt),
    ts: data.ts ? toIso(data.ts) : undefined,
  } as T;
}
...
export class FirestoreCustomerRepository implements CustomerRepository {
  ...
  async getHouseholdProfile(
    ...
    return {
      ...data,
      id: snapshot.id,
      lastConfirmedAt: toIso(data.lastConfirmedAt),
      createdAt: toIso(data.createdAt),
      updatedAt: toIso(data.updatedAt),
    } as HouseholdTravelProfile;
  }
  ...
  async listTravelHistory(customerId: string) {
    ...
    return snapshot.docs
      .filter((document) => document.data().orgId === this.orgId)
      .map((document) => mapNested<TravelHistory>(document));
  }
  ...
  async listEvents(customerId: string) {
    ...
    return snapshot.docs
      .filter((document) => document.data().orgId === this.orgId)
      .map((document) => mapNested<CustomerEvent>(document));
  }
  ...
  async previewImport(
    ...
    if (input.orgId !== this.orgId)
      throw new Error("Import organization does not match the active session.");
    ...
    const rows = input.rows.map((row, index) => {
      ...
      return {
        normalized,
        candidates: normalized.errors.length
          ? []
          : findDuplicateCandidates(normalized, existing),
      };
    });
    const stats = {
      total: rows.length,
    ...
    await importRef.set({
      ...
      dedupReport: rows
        ...
        .map((row) => ({
          ...
          candidateCustomerIds: row.candidates.map(
    ...
    for (let offset = 0; offset < rows.length; offset += 400) {
      ...
      rows
        ...
        .forEach((row) =>
          batch.set(
            importRef.collection("rows").doc(String(row.normalized.rowNumber)),
            row,
          ),
        );
      await batch.commit();
    ...
    const candidateIds = new Set(
      rows.flatMap((row) =>
        row.candidates.map((candidate) => candidate.customerId),
      ),
    );
    const candidateCustomers = Object.fromEntries(
      existing
        .filter((customer) => candidateIds.has(customer.id))
        .map((customer) => [
          customer.id,
          {
            name: customer.name,
  ...
  async commitImport(
    ...
    await this.database.runTransaction(async (transaction) => {
      ...
      if (!snapshot.exists || snapshot.data()?.orgId !== this.orgId)
        throw new Error("Import job was not found.");
      if (snapshot.data()?.status !== "review")
        throw new Error("This import has already been submitted.");
      const expectedRows = Number(snapshot.data()?.stats?.total || 0);
      if (
        input.decisions.length !== expectedRows ||
        new Set(input.decisions.map((decision) => decision.rowNumber)).size !==
          expectedRows
      ) {
    ...
    try {
      const rowSnapshots = await Promise.all(
        input.decisions.map((decision) =>
          importRef.collection("rows").doc(String(decision.rowNumber)).get(),
        ),
      );
      const rowByNumber = new Map(
        rowSnapshots
          .filter((row) => row.exists)
          .map((row) => [Number(row.id), row.data()!]),
      );
      const mergeIds = [
        ...new Set(
          input.decisions
      ...
      const mergeSnapshots = mergeIds.length
        ...
        : [];
      const mergeCustomers = new Map(
        mergeSnapshots
          .filter(
            (snapshot) =>
              snapshot.exists && snapshot.data()?.orgId === this.orgId,
          )
      ...
      for (const decision of input.decisions) {
        ...
        if (!row || row.errors?.length || decision.action === "skip") {
          skipped += 1;
          continue;
        }
          ...
          if (
            ...
            !review.candidates.some(
              (candidate: { customerId?: string }) =>
                candidate.customerId === decision.customerId,
            )
          ) {
            throw new Error(
              `Merge target ${decision.customerId} was not approved during duplicate review.`,
            );
          }
          const current = mergeCustomers.get(decision.customerId!);
          if (!current)
            throw new Error(
              `Merge target ${decision.customerId} is unavailable.`,
            );
      ...
      for (const [id, after] of mergeCustomers) {
        const before =
          mergeSnapshots.find((snapshot) => snapshot.id === id)?.data() || null;
        if (JSON.stringify(before) !== JSON.stringify(after))
          writes.push({
            ref: this.database.collection("customers").doc(id),
            data: after,
            before,
            action: "customer.import.merge",
          });
      }
      ...
      await importRef.update({
        status: "completed",
        stats: {
          ...(await importRef.get()).data()?.stats,
          created,
          updated,
          skipped,
        },
