    ...
    items: [
      {
        ...
        icon: Megaphone,
        permission: "marketing:read" as const,
      },
      {
        label: "Offers",
      ...
      {
        label: "Propensity explorer",
        href: "/admin/marketing/propensity",
        icon: Target,
        permission: "marketing:read" as const,
      },
      {
...
export function AdminShell({
  ...
  const searchableItems = visibleGroups.flatMap((group) =>
  ...
  useEffect(() => {
    if (window.localStorage.getItem("tlc-admin-collapsed") === "true")
      setCollapsed(true);
  ...
  function toggleCollapsed() {
    ...
    setCollapsed(next);
    window.localStorage.setItem("tlc-admin-collapsed", String(next));
  }
  ...
  return (
    <div
      className={`admin-shell${collapsed ? " is-collapsed" : ""}${mobileOpen ? " mobile-open" : ""}`}
    >
      <button
          ...
          {visibleGroups.map((group) => (
            <section key={group.label}>
              ...
              {group.items.map(({ label, href, icon: Icon }) => {
                const active =
                  pathname === href ||
                  (!["/admin", "/admin/content"].includes(href) &&
      ...
      <div className="admin-workspace">
        <header className="admin-topbar">
          ...
          <div className="admin-topbar-actions">
                ...
                <div className="admin-quick-menu">
                  {hasPermission(user.role, "crm:write") && (
                    <Link href="/admin/crm/new">
                      ...
                      <span>
                        <b>New lead</b>
                        <small>Capture an opportunity</small>
                      </span>
                    </Link>
                  )}
                  {hasPermission(user.role, "quotes:write") && (
                    <Link href="/admin/quotes/new">
                      <FileText />
                      <span>
                        <b>Build quote</b>
                        <small>Create an itinerary</small>
                      </span>
                    </Link>
                  )}
                  {hasPermission(user.role, "quotes:write") && (
                    <Link href="/admin/inventory">
                      ...
                      <span>
                        ...
                        <small>Flights and hotels</small>
                      </span>
                    </Link>
                  )}
                  {hasPermission(user.role, "content:write") && (
                    <Link href="/admin/content/trips/new">
                      <LibraryBig />
                      <span>
                        <b>New package</b>
                        <small>Publish a website tour</small>
                      </span>
                    </Link>
                  )}
                </div>
            ...
            <Link
              className="admin-alert-shortcut"
              href="/admin/alerts"
      ...
      {commandOpen && (
        ...
        >
          <section
            ...
            role="dialog"
            aria-modal="true"
            aria-label="Search TLC Operations"
          ...
          >
            ...
            <div>
              ...
              {matches.map(({ label, href, icon: Icon, group }) => (
              ...
              {!matches.length && (
                <div className="admin-command-empty">
                  <Activity />
                  <b>No matching workspace</b>
                  <span>Try “customer”, “booking”, or “finance”.</span>
                </div>
              )}
            </div>
            <footer>
              <span>Type to filter workspaces</span>
              <span>
                <kbd>esc</kbd> Close
              </span>
            </footer>
