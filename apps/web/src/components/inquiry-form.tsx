...
import {
  createTravelIntelligence,
  TravelProfileCapture,
} from "@/components/travel-profile-capture";
...
export function InquiryForm({
  ...
  const [intelligence, setIntelligence] = useState(() =>
    createTravelIntelligence({
      destinations: defaults?.destinationIds,
      month: defaults?.travelMonth,
      travellerType: defaults?.travellerType,
    }),
  );
  const {
    register,
  ...
  const submit = handleSubmit(async (values) => {
    setServerError("");
    const profile = travelIntelligenceInputSchema.safeParse(intelligence);
    if (!profile.success) {
      setServerError(
        profile.error.issues[0]?.message || "Please check your travel profile.",
      );
      return;
    }
    ...
    try {
      ...
      if (!response.ok)
        throw new Error(payload.error || "We couldn't save your request.");
      setComplete(true);
  ...
  if (complete) {
    return (
      <div className="inquiry-success planner-fade-in" role="status">
        <CheckCircle2 />
        <p className="eyebrow">Request received</p>
        <h2>We’ll take it from here.</h2>
        <p>
          A TLC travel expert can now review your requirements and contact you
  ...
  return (
    <form
      className={`inquiry-form ${compact ? "inquiry-compact" : ""}`}
      onSubmit={submit}
      noValidate
    >
      <div className="form-heading">
        <p className="eyebrow">Plan with a person</p>
        <h2>{title}</h2>
        <p>{description}</p>
      </div>
      <div className="form-grid">
        ...
        <label className="form-wide">
          ...
          <textarea
            ...
            placeholder="Destination, dates, travellers, budget—or simply the kind of break you need."
          />
          {errors.requirements && <small>{errors.requirements.message}</small>}
        </label>
      ...
      <p className="form-note">
        TLC uses your choices only as permitted above. Marketing consent is
        separate, and no passport or payment details are requested here.
      </p>
    </form>
  );
}
