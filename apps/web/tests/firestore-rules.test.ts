...
beforeAll(async () => {
  environment = await initializeTestEnvironment({
    ...
    firestore: { host: "127.0.0.1", port: 8080, rules: readFileSync(new URL("../../../firebase/firestore.rules", import.meta.url), "utf8") },
  });
  await environment.withSecurityRulesDisabled(async (context) => {
    await setDoc(doc(context.firestore(), "destinations", "published"), { status: "published", name: "Published" });
    await setDoc(doc(context.firestore(), "destinations", "draft"), { status: "draft", name: "Draft" });
    await setDoc(doc(context.firestore(), "inquiries", "one"), { orgId: "tlc-vacations", status: "new", assignedUid: "sales", customer: { fullName: "Test" } });
    await setDoc(doc(context.firestore(), "leads", "other-salesperson"), { orgId: "tlc-vacations", status: "new", assignedUid: "other" });
    await setDoc(doc(context.firestore(), "leads", "assigned-sales"), { orgId: "tlc-vacations", status: "new", assignedUid: "sales" });
    await setDoc(doc(context.firestore(), "leads", "other-org"), { orgId: "another-org", status: "new", assignedUid: "sales" });
...
describe("public and content rules", () => {
  ...
  it("allows editors to manage content but not read CRM intake", async () => {
    const database = environment.authenticatedContext("editor", { role: "content_editor" }).firestore();
    await assertSucceeds(setDoc(doc(database, "destinations", "new"), { status: "draft", name: "New" }));
...
describe("CRM and audit rules", () => {
  it("rejects browser-created public inquiries", async () => {
    const database = environment.unauthenticatedContext().firestore();
    await assertFails(setDoc(doc(database, "inquiries", "public"), { status: "new" }));
  });

  it("allows sales staff to read inquiries and create leads", async () => {
    const database = environment.authenticatedContext("sales", { role: "sales", orgId: "tlc-vacations" }).firestore();
    await assertSucceeds(getDoc(doc(database, "inquiries", "one")));
    await assertSucceeds(setDoc(doc(database, "leads", "new"), { orgId: "tlc-vacations", status: "new", assignedUid: "sales" }));
  ...
  it("allows assigned staff to append but not rewrite lead activity", async () => {
    const database = environment.authenticatedContext("sales", { role: "sales", orgId: "tlc-vacations" }).firestore();
    const activity = doc(database, "leads", "assigned-sales", "activities", "call-one");
    await assertSucceeds(setDoc(activity, { orgId: "tlc-vacations", leadId: "assigned-sales", type: "call", body: "Called customer" }));
  ...
  it("keeps audit logs immutable to browser clients", async () => {
    ...
    await assertFails(setDoc(doc(database, "auditLogs", "browser"), { action: "forged" }));
  });
  ...
  it("lets managers review imports but keeps import writes server-owned", async () => {
    ...
    await assertFails(setDoc(doc(database, "imports", "forged"), { orgId: "tlc-vacations", status: "completed" }));
  });
});
