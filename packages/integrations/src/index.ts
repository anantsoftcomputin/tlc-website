...
export class MockProvider implements HealthCheckableProvider {
  readonly key = "mock";
  async healthCheck() {
    return { ok: true, reasoning: "Deterministic local mock is available." };
  }
}
...
export class MockWhatsAppInboundAdapter implements InboundLeadAdapter<
    ...
    return [
      {
        ...
        customer: {
          fullName: String(payload.fullName || "WhatsApp traveller"),
          phone: String(payload.phone || ""),
        },
        destinations: Array.isArray(payload.destinations)
          ? payload.destinations.map(String)
          : [],
        requirements: payload.message ? String(payload.message) : undefined,
...
export class GmailInboundAdapter implements InboundLeadAdapter<
  Record<string, unknown>
> {
  readonly key = "gmail";
  async verify(
    ...
    return Boolean(headers["x-goog-channel-token"]);
  }
  async parse(payload: Record<string, unknown>) {
    return [
      {
        externalId: String(payload.messageId || payload.id || ""),
        source: "email" as const,
        receivedAt: String(payload.receivedAt || new Date().toISOString()),
        customer: {
          fullName: String(payload.fromName || "Email traveller"),
          phone: String(payload.phone || ""),
          ...(payload.fromEmail ? { email: String(payload.fromEmail) } : {}),
        },
        destinations: Array.isArray(payload.destinations)
          ? payload.destinations.map(String)
          : [],
...
export class SocialInboundAdapter implements InboundLeadAdapter<
  Record<string, unknown>
> {
  readonly key = "social";
  async verify(
    ...
    return [
      {
        ...
        receivedAt: String(payload.receivedAt || new Date().toISOString()),
        customer: {
          fullName: String(payload.fullName || "Social traveller"),
          phone: String(payload.phone || ""),
        },
        destinations: Array.isArray(payload.destinations)
          ? payload.destinations.map(String)
