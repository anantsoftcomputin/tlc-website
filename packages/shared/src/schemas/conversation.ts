...
import { assistantResponseEnvelopeSchema } from "./assistant-experience.js";
  ...
  .object({
    ...
    participants: z
      .array(
        z.object({
          id: documentIdSchema,
          type: z.enum(["customer", "staff", "bot"]),
...
export const conversationMessageSchema = z
  .object({
    ...
    from: z.object({
      id: documentIdSchema,
      type: z.enum(["customer", "staff", "bot", "system"]),
    }),
    body: z.string().max(20000),
    inputMode: z.enum(["text", "voice", "tap", "system"]).default("text"),
    transcript: z
      .object({
    ...
    toolCalls: z
      ...
      .default([]),
    experience: assistantResponseEnvelopeSchema.optional(),
  ...
  .superRefine((message, context) => {
    if (message.aiGenerated && !message.reasoning) {
      context.addIssue({
        code: "custom",
        path: ["reasoning"],
        message: "AI-generated messages require reasoning.",
      });
    }
  });
