...
import { taxProfileSchema } from "./finance.js";
...
export const leadAssignmentSettingsSchema = z.object({
  mode: z
    .enum(["manual", "round_robin", "destination_specialist"])
    .default("manual"),
  ...
  .object({
    ...
    settings: z.object({
      ...
      discountLimits: z.object({
        salesPct: z.number().min(0).max(100),
        managerPct: z.number().min(0).max(100),
      }),
      automation: automationSettingsSchema,
      leadAssignment: leadAssignmentSettingsSchema.default({
        mode: "manual",
        eligibleUids: [],
        destinationOwners: {},
